# Validation report

Generated: 2026-09-11T15:45:55.434Z · seed `20260902` · independent re-verification: `scripts/verify_bank.py`

## Summary

| Metric | Value |
|---|---|
| Questions generated | 660 |
| Questions valid (shown to students) | 638 |
| Questions rejected | 22 |
| Rejection rate | 3.33 % |
| Ambiguity rate (two options denoting the same value) | 0.00 % |
| Duplicate rate (identical item signature) | 2.42 % (16 items) |
| Template repetition rate (variant cap applied) | 0.91 % |
| Answer-key failures (solver vs. declared key) | 0 |
| Solver disagreements (all) | 0 |
| Numeric items independently solved | 381 |
| Structural failures | 0 |

## Rejection reasons

| Reason | Count |
|---|---|
| duplicate item (exact signature) | 16 |
| template repetition (variant cap reached) | 6 |

## Coverage by domain

| Domain | Generated | Valid | Levels present |
|---|---|---|---|
| D01 | 54 | 54 | 2, 3, 5 |
| D02 | 135 | 134 | 1, 2, 3, 4, 5, 6 |
| D03 | 29 | 27 | 2, 3, 6 |
| D04 | 39 | 37 | 2, 4 |
| D05 | 13 | 13 | 2 |
| D06 | 122 | 120 | 1, 2, 3, 4, 5, 7 |
| D07 | 26 | 25 | 2, 3 |
| D08 | 37 | 33 | 2, 3 |
| D09 | 109 | 100 | 2, 3, 4, 5, 6, 7 |
| D10 | 37 | 36 | 3, 4, 5 |
| D11 | 13 | 13 | 1, 2, 3, 4, 7 |
| D12 | 12 | 12 | 2, 3, 4, 5, 6 |
| D13 | 21 | 21 | 4, 5, 6 |
| D14 | 6 | 6 | 3, 4 |
| D15 | 7 | 7 | 2, 3, 4, 5, 6 |

## Coverage by difficulty level

| Level | Items |
|---|---|
| 1 | 31 |
| 2 | 206 |
| 3 | 197 |
| 4 | 112 |
| 5 | 74 |
| 6 | 15 |
| 7 | 3 |

## Rejected items (nothing hidden)

_None._

## Generator-level retries

Option sets rejected inside the generators (fresh random values were drawn):

| Family | Rejected draws |
|---|---|
| physics/gas | 156 |
| engineering/lever | 156 |
| math/percentage | 1 |
| vector/area | 1 |
| math/proportion | 1 |
| economics/break-even | 1 |
| computing/binary | 1 |

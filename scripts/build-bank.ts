...
import { LESSONS, lessonProblems, lessonStats } from '../content/lessons';

const SEED = 20260902;
const EXAM_SEED = 20260926;

const { bank, report, reportMarkdown } = buildBank({ seed: SEED, variantsPerFamily: 14, targetCandidates: 660, lessons: LESSONS });
...
/* ------------------------------------------------------------------ lessons */
const lessonIssues = lessonProblems();
const lStats = lessonStats();
console.log('');
console.log('=== LESSON LAYER (12-step teaching sequence) ===');
console.log('lessons    :', LESSONS.length, '· every lesson has all 12 steps and practice pools fed by the bank');
console.log('minutes    :', lStats.reduce((s, l) => s + l.minutes, 0), 'total guided time');
console.log('problems   :', lessonIssues.length === 0 ? 'none' : lessonIssues.join(' | '));

const mdLessons: string[] = [];
mdLessons.push('# 05 — Lesson layer', '');
mdLessons.push(
  'Generated by `scripts/build-bank.ts`. Every lesson follows the mandated sequence: what it is → why it matters → prerequisites → core knowledge → visual intuition → worked example → guided practice → independent practice → transfer → dMAT-style items → trick/misconception → mastery check. ' +
    'Each practice step declares a concept pool and a difficulty band; the UI draws its items live from `content/bank.json`, so teaching content and item content can never drift apart.',
  '',
);
mdLessons.push('| Lesson | Domain | Sections | Practice pools | Levels | Minutes |', '|---|---|---|---|---|---|');
for (const l of lStats) mdLessons.push(`| ${l.lessonId} | ${l.domainId} | ${l.sections} | ${l.practicePools} | ${l.levels.join(', ')} | ${l.minutes} |`);
mdLessons.push('', `Integrity check: ${lessonIssues.length === 0 ? 'no problems — every concept in the curriculum graph points at an existing lesson, in the right domain, and is practised in that lesson.' : lessonIssues.join('; ')}`);
mdLessons.push('');
...
console.log('bank lessons:', bank.lessons.length);
...
writeFileSync('docs/05_LESSONS.md', mdLessons.join('\n'));
